...
const CaseHistory = ({
  ...
  initialGateKeeperName = '',
  initialGateKeeperPhone = '',
  initialCustomerAvailability = '',
  callHistory = [], // This prop is no longer used in this component
}) => {
  const [caseDescription, setCaseDescription] = useState('');
  const [caseResult, setCaseResult] = useState('Intrested');
  ...
  // This component's tabs are explicitly limited to 'caseSummary' and 'createLead'
  const [activeInternalTab, setActiveInternalTab] = useState('caseSummary');
  const [caseCreated, setCaseCreated] = useState(false);

  // Effect to update local state if initial props change (e.g., when a new case ID is loaded)
  useEffect(() => {
    setShopOwnerName(initialCustomerName);
    setShopOwnerPhone(initialCustomerPhone);
    setGateKeeperName(initialGateKeeperName);
    setGateKeeperPhone(initialGateKeeperPhone);
    // If initialCustomerAvailability was a JSON string, you'd parse it here:
    // try {
    //   if (initialCustomerAvailability) {
    //     setAvailability(JSON.parse(initialCustomerAvailability));
    //   }
    // } catch (e) {
    //   console.error("Failed to parse initialCustomerAvailability:", e);
    // }
  }, [initialCustomerName, initialCustomerPhone, initialGateKeeperName, initialGateKeeperPhone, initialCustomerAvailability]);
  ...
  const handleCreateCase = () => {
    ...
    Swal.fire({
      icon: 'info',
      title: 'Case Created!',
      text: 'You can now fill in case details.',
      background: isDarkMode ? '#4A5568' : '#fff',
      color: isDarkMode ? '#E2E8F0' : '#1A202C',
  ...
  return (
    <div className={formClasses.container}>
      ...
      {/* Tab-like buttons for CaseHistory (right card) */}
      <div className="flex mb-4 -mt-2 -mx-4 px-4 pt-2 border-b border-gray-600">
        <button
          className={`py-2 px-4 text-sm font-medium focus:outline-none ${
            activeInternalTab === 'caseSummary'
              ? 'border-b-2 border-blue-500 text-blue-500'
              : `${isDarkMode ? 'text-gray-400 hover:text-gray-200' : 'text-gray-600 hover:text-gray-800'}`
          }`}
          onClick={() => { setActiveInternalTab('caseSummary'); }}
        >
          Case Summary
        </button>
        <button
        ...
        {/* The 'Activity History' tab is explicitly removed from here */}
      </div>
      ...
      ) : ( // This is the main Case Summary form
        <>
          <div className={formClasses.scrollableArea}>
            <form onSubmit={handleSubmit} className="flex flex-col h-full">
              ...
              <div className="mb-2 flex flex-col sm:flex-row sm:items-center">
                ...
                {/* Display initialCustomerAvailability if it exists and is a string */}
                {initialCustomerAvailability && !hasAnyAvailabilitySet() && (
                    <div className="mt-2 sm:mt-0 sm:ml-3 text-sm text-gray-400">
                        Initial Availability: {initialCustomerAvailability}
                    </div>
                )}
              </div>
              ...
              {/* Case Description */}
              <div className="mb-3">
                <label htmlFor="caseDescription" className={formClasses.label}>
                  Case Description<span className={formClasses.requiredStar}>*</span>
                </label>
                <textarea
                  id="caseDescription"
                  rows="3"
              ...
              {/* Select Case Result */}
              <div className="mb-3">
                <label htmlFor="caseResult" className={formClasses.label}>
                  Select Case Result<span className={formClasses.requiredStar}>*</span>
                </label>
